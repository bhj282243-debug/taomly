...
-- Создаёт таблицы биллинга и заполняет стартовые тарифные планы.
--
-- Таблицы: subscription_plans, subscriptions, usage_events
-- Seed:    Free, Basic, Pro планы (минимальный рабочий набор)
--
-- Когда применять:
--   Если таблицы биллинга ещё не созданы в Neon.
--   Выполнить ВРУЧНУЮ в Neon SQL Editor.
--
-- Идемпотентность:
--   CREATE TABLE IF NOT EXISTS — не упадёт при повторном запуске.
--   INSERT ... ON CONFLICT DO NOTHING — не дублирует тарифы.
--
-- ВАЖНО: таблица restaurants должна существовать до выполнения этой миграции.
-- ─────────────────────────────────────────────────────────────────────────────
